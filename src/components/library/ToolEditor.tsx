...
import type { ToolType, ToolUnit, CoolantMode, FeedMode } from '../../types/tool';
...
function NumF({
  value, onChange, min, step = 'any',
}: {
  value: number | undefined; onChange: (v: number | undefined) => void; min?: number; step?: string | number;
}) {
  return (
    <input
      type="number"
...
export default function ToolEditor({
  ...
  const [draft,       setDraft]       = useState<LibraryTool>(() =>
    tool ? { ...tool } : makeBlankTool(unit, settings),
  );
  ...
  useEffect(() => {
    setDraft(tool ? { ...tool } : makeBlankTool(unit, settings));
  ...
  const machineOptions = [
    { value: '', label: '— None —' },
    ...allMachineGroups.map((g) => ({ value: g, label: g })),
    ...(newGroup ? [{ value: newGroup, label: newGroup }] : []),
  ];
    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-[480px] bg-slate-800 border-l border-slate-700 z-50 flex flex-col shadow-2xl">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700 shrink-0">
          ...
          <button onClick={onClose} className="p-1.5 rounded text-slate-400 hover:text-white hover:bg-slate-700 transition-colors">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {activeTab === 'library' && (
            <>
              <div className="grid grid-cols-2 gap-3">
                <Row2 label="Tool number (T#)">
                  <NumF
                    ...
                    min={0}
                    step={1}
                    onChange={(v) => patchDraft({ toolNumber: v ?? 1 })}
                  />
                </Row2>
                <Row2 label="Unit">
                  <SelF
              ...
              <Row2 label="Description">
                <TextF value={draft.description} onChange={(v) => patchDraft({ description: v })} placeholder="e.g. 6mm 2-flute flat end mill" />
              </Row2>

              <Row2 label="Manufacturer">
                <TextF value={draft.manufacturer ?? ''} onChange={(v) => patchDraft({ manufacturer: v || undefined })} placeholder="e.g. Kyocera" />
              </Row2>

              <Row2 label="Product ID / Part #">
                <TextF value={draft.productId ?? ''} onChange={(v) => patchDraft({ productId: v || undefined })} />
              </Row2>

              <Row2 label="Comment">
                <TextF value={draft.comment ?? ''} onChange={(v) => patchDraft({ comment: v || undefined })} />
              </Row2>
              ...
              <Row2 label="Machine group">
                <div className="space-y-1.5">
                  <SelF
                    ...
                    onChange={(v) => patchDraft({ machineGroup: v || undefined })}
                  />
                  <input
                    type="text"
                    value={newGroup}
                    ...
                    onChange={(e) => {
                      ...
                      if (e.target.value) patchDraft({ machineGroup: e.target.value });
                    }}
                    className="w-full px-2.5 py-1.5 text-xs bg-slate-700 border border-slate-600 rounded-lg text-slate-300 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                </div>
              </Row2>
          ...
          {activeTab === 'geometry' && (
            <>
              <div className="grid grid-cols-2 gap-3">
                <Row2 label="Diameter *"><NumF value={geo.diameter} onChange={(v) => patchGeo({ diameter: v ?? 0 })} min={0} /></Row2>
                <Row2 label="Shaft diameter"><NumF value={geo.shaftDiameter} onChange={(v) => patchGeo({ shaftDiameter: v })} min={0} /></Row2>
                <Row2 label="Overall length"><NumF value={geo.overallLength} onChange={(v) => patchGeo({ overallLength: v })} min={0} /></Row2>
                <Row2 label="Body length"><NumF value={geo.bodyLength} onChange={(v) => patchGeo({ bodyLength: v })} min={0} /></Row2>
                <Row2 label="Flute length"><NumF value={geo.fluteLength} onChange={(v) => patchGeo({ fluteLength: v })} min={0} /></Row2>
                <Row2 label="Shoulder length"><NumF value={geo.shoulderLength} onChange={(v) => patchGeo({ shoulderLength: v })} min={0} /></Row2>
                <Row2 label="Number of flutes"><NumF value={geo.numberOfFlutes} onChange={(v) => patchGeo({ numberOfFlutes: v })} min={0} step={1} /></Row2>
                <Row2 label="Corner radius"><NumF value={geo.cornerRadius} onChange={(v) => patchGeo({ cornerRadius: v })} min={0} /></Row2>
                <Row2 label="Taper angle (°)"><NumF value={geo.taperAngle} onChange={(v) => patchGeo({ taperAngle: v })} min={0} /></Row2>
                <Row2 label="Tip diameter"><NumF value={geo.tipDiameter} onChange={(v) => patchGeo({ tipDiameter: v })} min={0} /></Row2>
              </div>
          ...
          {activeTab === 'cutting' && (
            <>
              <div className="grid grid-cols-2 gap-3">
                <Row2 label="Spindle RPM"><NumF value={cut.spindleRpm} onChange={(v) => patchCut({ spindleRpm: v })} min={0} step={1} /></Row2>
                <Row2 label="Cutting feed"><NumF value={cut.feedCutting} onChange={(v) => patchCut({ feedCutting: v })} min={0} /></Row2>
                <Row2 label="Plunge feed"><NumF value={cut.feedPlunge} onChange={(v) => patchCut({ feedPlunge: v })} min={0} /></Row2>
                <Row2 label="Ramp feed"><NumF value={cut.feedRamp} onChange={(v) => patchCut({ feedRamp: v })} min={0} /></Row2>
                <Row2 label="Entry feed"><NumF value={cut.feedEntry} onChange={(v) => patchCut({ feedEntry: v })} min={0} /></Row2>
                <Row2 label="Retract feed"><NumF value={cut.feedRetract} onChange={(v) => patchCut({ feedRetract: v })} min={0} /></Row2>
              </div>
        ...
        <div className="px-5 py-4 border-t border-slate-700 shrink-0 flex items-center gap-3">
          ...
          <button
            ...
            className={[
              ...
              isSaving || !draft.description.trim()
